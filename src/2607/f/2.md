...
# F2 Установка и использование Logisim
...
-->
Logisim — это программа для графического проектирования цифровых схем, которая позволяет пользователю проектировать и моделировать цифровые схемы, перетаскивая нужные компоненты схемы. Мы будем использовать Logisim-evolution — обновлённую версию Logisim с более широким набором компонентов. Однако для простоты изложения далее мы будем использовать `Logisim` для обозначения этой обновлённой версии.
...
-->
> #### comment::Преимущества программного моделирования
>
> Использование программ моделирования, таких как Logisim, для проектирования схем даёт следующие преимущества:
>
> * **Бесплатно:** не нужно закупать настоящее оборудование и компоненты.
> * **Гибко:** доступно неограниченное количество компонентов, и проектирование схемы не ограничено размером макетной платы.
> * **Компоненты не изнашиваются:** если вы пользовались макетной платой, вы, возможно, испытывали разочарование от многочасовой отладки из-за повреждённых компонентов.
> * **Безопасно:** не нужно беспокоиться о коротких замыканиях и других опасных событиях, которые могут привести к авариям.
...
-->
Logisim-evolution — это Java-приложение, способное работать на любой операционной системе, поддерживающей среду выполнения Java (JRE).  
Хотя она совместима с такими распространёнными системами, как Windows, Mac OS и Linux, текущая программа обучения не требует обязательного использования операционной системы Linux.  
Если у вас есть опыт работы с системами на основе Linux и вы уверенно решаете вопросы настройки окружения, вы можете использовать версию программы для Linux.

Logisim-evolution требует Java версии 16 или выше, и мы рекомендуем использовать Java версии 17 для оптимальной совместимости и производительности.  
Если вы хотите попробовать более новые версии, такие как Java 21 или Java 22, вы можете скачать соответствующие версии тем же способом.

Вы можете перейти на [страницу загрузки на сайте Oracle][java download] и выбрать подходящую версию в зависимости от вашей операционной системы.  
Для пользователей Windows обычно доступны три разных варианта загрузки:
`x64 Compressed Archive`, `x64 Installer`, `x64 MSI Installer`.
Мы рекомендуем использовать `x64 Installer` за его простоту: он не требует ручной настройки пользователем и позволяет напрямую указать каталог установки. Этот способ обеспечивает быструю настройку с минимальным вмешательством пользователя, что делает его идеальным для тех, кто хочет простой процесс установки.
...
-->
Logisim-evolution поддерживает два способа установки: через менеджер пакетов и через сборку из исходного кода. Здесь мы рекомендуем использовать способ через менеджер пакетов за его простоту и эффективность.

Сначала перейдите на [страницу релизов проекта Logisim-evolution][logisim release].
Затем найдите последнюю версию релиза, V4.1.0, и в разделе `Assets` выберите пакет установки, совместимый с вашей операционной системой. Нажмите на соответствующий файл, чтобы начать загрузку.
Для пользователей **систем Windows на архитектуре x86** выберите `logisim-evolution-4.1.0-amd64.msi`;
Для **пользователей Mac OS** проверьте, установлен ли на вашем Mac чип Apple Silicon (серии M). Если да, выберите `logisim-evolution-4.1.0-aarch64.dmg`. Если на вашем Mac процессор Intel, выберите `logisim-evolution-4.1.0-x86_64.dmg`.

Если на вашем компьютере не установлена среда выполнения Java, установите и настройте её самостоятельно.

Затем запустите `logisim-evolution-4.1.0-amd64.msi` (Windows) или `logisim-evolution-4.1.0.dmg` (macOS), чтобы установить Logisim.
...
-->
> #### todo::Установите Logisim
>
> Установите Logisim, следуя описанным выше шагам. После установки попробуйте открыть Logisim и проверьте, что программа корректно работает.

<!-- ## 探索Logisim -->

<!-- ## Explore Logisim -->
## Знакомство с Logisim
...
-->
> #### option::Переключение языка
>
> Если вы достаточно хорошо читаете по-английски, мы всё же рекомендуем вам продолжать пользоваться английским языком.
> Если вы хотите переключиться на китайский или другой язык, откройте `File->Preferences` в меню, перейдите на вкладку `International` во всплывающем диалоговом окне и установите `Language` в значение `Chinese` или другой язык.
>
> Однако в некоторых переводах используется нестандартная терминология.
> Поэтому в последующих хендаутах по Logisim мы будем в основном давать английскую терминологию.
...
-->
> #### todo::Спроектируйте свою первую цифровую схему в Logisim
>
> Ваша первая задача — RTFM.
> Нажмите `Help->Tutorual` в меню — откроется вводный учебник из официальной документации Logisim.
> Учебник состоит из 6 частей: вам нужно читать учебник и одновременно выполнять соответствующие действия в Logisim, чтобы освоиться с использованием Logisim.
>
> При необходимости вы можете переключиться обратно на английский язык и прочитать оригинальный учебник, чтобы точно понять его смысл.
...
-->
> #### todo::Изучите графический интерфейс Logisim
>
> Прочитайте раздел `The graphical interfaces` и раздел `Menu reference` официальной документации Logisim, чтобы познакомиться с некоторыми возможностями и базовыми операциями.
...
-->
Входы и выходы очень важны для модуля схемы.
Выходы позволяют пользователю напрямую наблюдать результаты работы схемы, а изменение входов позволяет проверить, что схема ведёт себя как ожидается в различных состояниях.
После краткого знакомства с Logisim здесь представлены некоторые распространённые компоненты ввода и вывода.

* Входы: Button, Dip Switch
* Выходы: LED, 7-Segment Display
...
-->
> #### todo::RTFM, чтобы разобраться в назначении компонентов
>
> Найдите в оглавлении официальной документации `Library Reference` -> Input/Output library и прочитайте о назначении перечисленных выше компонентов ввода/вывода.
...
-->
> #### todo::Спроектируйте в Logisim схему, которая вам интересна
>
> Освоив базовые приёмы работы в Logisim с помощью учебников, попробуйте изучить остальные компоненты и функции Logisim.
> Затем, включив воображение, спроектируйте цифровую схему, которая покажется вам интересной, и попробуйте запустить её моделирование, чтобы проверить, соответствует ли результат вашим ожиданиям.
